<?php
// $provider fornito dal controller
?>
<!DOCTYPE html>
<html lang="it">
<head>    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Modifica Provider - CoreSuite</title>
    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Modifica Provider #<?= htmlspecialchars($provider['id']) ?></h1>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="col-md-8">
                        <div class="card card-warning">
                            <div class="card-header">
                                <h3 class="card-title">Modifica dati provider</h3>
                            </div>
                            <form method="post">
                                <div class="card-body">                                    <div class="form-group">
                                        <label for="name">Nome</label>
                                        <input type="text" name="name" id="name" class="form-control" value="<?= htmlspecialchars($provider['name']) ?>" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="code">Codice</label>
                                        <input type="text" name="code" id="code" class="form-control" value="<?= htmlspecialchars($provider['code']) ?>">
                                    </div>
                                    <div class="form-group">
                                        <label for="type">Tipo</label>
                                        <select name="type" id="type" class="form-control" required>
                                            <option value="telefonia" <?= $provider['type']==='telefonia'?'selected':'' ?>>Telefonia</option>
                                            <option value="energia" <?= $provider['type']==='energia'?'selected':'' ?>>Energia</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="logo">Logo (URL o percorso)</label>
                                        <input type="text" name="logo" id="logo" class="form-control" value="<?= htmlspecialchars($provider['logo']) ?>">
                                    </div>
                                    <div class="form-group">
                                        <label for="form_config">Configurazione Form Dinamico (JSON)</label>
                                        <textarea name="form_config" id="form_config" class="form-control"><?= htmlspecialchars($provider['form_config']) ?></textarea>
