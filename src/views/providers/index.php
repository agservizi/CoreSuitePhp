<?php
// $providers fornito dal controller
?>
<!DOCTYPE html>
<html lang="it">
<head>    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Gestori/Provider - CoreSuite</title>
    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Elenco Gestori/Provider</h1>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="container-fluid">
                ...
                <div class="mb-3 d-flex flex-wrap align-items-center justify-content-between">
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                            <a href="/providers_create.php" class="btn btn-primary mb-2 mr-2"><i class="fas fa-plus"></i> Nuovo Provider</a>
                            <a href="/export_providers.php" class="btn btn-success mb-2"><i class="fas fa-file-csv"></i> Esporta CSV</a>
                        <?php endif; ?>
                    </div>
                    <form class="form-inline mb-2" id="searchForm" onsubmit="return false;">
                        <div class="input-group input-group-sm">
                            <input type="text" class="form-control" id="searchInput" placeholder="Cerca nome, tipo, ...">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Tutti i provider</h3>
                    </div>
                        ...
                        <table class="table table-striped table-hover" id="providersTable">
                            ...
                            <?php foreach ($providers as $p): ?>
                                <tr>
                                    ...
                                    <td><?php if ($p['logo']): ?><img src="<?= htmlspecialchars($p['logo']) ?>" alt="logo" style="height:32px;max-width:80px;"><?php endif; ?></td>
                                    <td>
                                        <a href="/providers_show.php?id=<?= $p['id'] ?>" class="btn btn-info btn-sm" title="Dettagli"><i class="fas fa-eye"></i></a>
                                        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                                            <a href="/providers_edit.php?id=<?= $p['id'] ?>" class="btn btn-warning btn-sm" title="Modifica"><i class="fas fa-edit"></i></a>
                                            <a href="/providers_delete.php?id=<?= $p['id'] ?>" class="btn btn-danger btn-sm" title="Elimina" onclick="return confirm('Sei sicuro di voler eliminare questo provider?');"><i class="fas fa-trash"></i></a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
...
$(function() {
    ...
    // Esempio: ?success=Provider+creato+con+successo
