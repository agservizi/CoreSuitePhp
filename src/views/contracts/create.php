...
<head>
    ...
    <title>Nuovo Contratto - CoreSuite</title>    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/admin-lte@3.2/dist/css/adminlte.min.css">
    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Nuovo Contratto</h1>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="card card-primary">
                            ...
                            <form method="post">
                                <div class="card-body">
                                    ...
                                    <div id="provider-extra-fields"></div>
                                </div>
                                <div class="card-footer">
                                    <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Crea Contratto</button>
...
function loadProviderForm() {
    ...
    var container = document.getElementById('provider-extra-fields');
    container.innerHTML = '';
    if (!providerId) return;
    fetch('/providers_show.php?id=' + providerId + '&json=1')
        .then(r => r.json())
        .then(data => {
            if (data.form_config) {
                try {
                    var fields = JSON.parse(data.form_config);
                    fields.forEach(f => {
                        var html = '<div class="form-group">';
                        html += '<label>' + f.label + '</label>';
                        html += '<input type="' + (f.type||'text') + '" name="extra[' + f.name + ']" ' + (f.required?'required':'') + ' class="form-control">';
                        html += '</div>';
